...
class EnfermeriaRobot:
    ...
    def _setup_environment(self):
        ...
        # Posiciones para los 3 bloques
        posiciones = [
            Point(x=0.5, y=-0.3, z=0.73),  # rojo
            Point(x=0.5, y=0.0, z=0.73),   # azul
            Point(x=-0.5, y=0.3, z=1),   
        ]

        for i in range(3):  # ahora son 3 bloques
            block = load_model(BLOCK_URDF, fixed_base=False)
            set_pose(block, Pose(posiciones[i]))
    ...
    def _setup_robot(self):
        ...
        joint_positions = [pi, pi/4, 0, 0, 0, 0, 0]
    ...
    def plan_pick_and_place(self, obj, destino_pose):
        ...
        ik_fn = get_ik_fn(self.robot, fixed=[self.floor, self.mesa, self.cama, self.estanteria], teleport=True)
    ...
    def plan_pick_and_place3(self, obj, destino_pose):
        ...
        grasp_gen = get_grasp_gen(self.robot, 'top')
        ik_fn = get_ik_fn(self.robot, fixed=[self.estanteria], teleport=False)
        free_motion_fn = get_free_motion_gen(self.robot, fixed=[self.estanteria], teleport=True)
        holding_motion_fn = get_holding_motion_gen(self.robot, fixed=[self.estanteria], teleport=False)
    ...
    def ejecutar_trayectoria_cartesiana(self, poses: list[Pose], delay: float = 20, steps_per_segment: int = 100):
        from pybullet_tools.utils import (
...
if __name__ == '__main__':
    ...
    radio = 0.5
    q = [0, pi/2, 0]
    poses.append(Pose(Point(-0.3, 0, 1.3), q))
    poses.append(Pose(Point(-0.5, 0.3, 1.3), q))
    ...
    '''
    time.sleep(30) 
    robot.camera_thread_running = False
    update_state()
    
    destino = Pose(Point(x=0.6, y=0.3, z=0.7))
